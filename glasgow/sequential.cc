                        ...
                        for (int d = nc.first_set_bit() ; d != -1 ; d = nc.first_set_bit()) {
                            ...
                            for (int e = nd.first_set_bit() ; e != -1 ; e = nd.first_set_bit()) {
                                ...
                                for (int w = ne.first_set_bit() ; w != -1 && unsigned(w) <= v ; w = ne.first_set_bit()) {
                                    ...
                                    if (k_ >= 3 && pattern_graphs.at(2 * k_ + 2).adjacent(v, w))
                                        pattern_graphs.at(2 * k_ + 3).add_edge(v, w);
                                    else if (k_ >= 2 && pattern_graphs.at(2 * k_ + 1).adjacent(v, w))
                                        pattern_graphs.at(2 * k_ + 2).add_edge(v, w);
                                    else if (k_ >= 1)
                                        pattern_graphs.at(2 * k_ + 1).add_edge(v, w);
                                }
                            }
                        }
                    }
                        ...
                        for (int d = nc.first_set_bit() ; d != -1 ; d = nc.first_set_bit()) {
                            ...
                            for (int e = nd.first_set_bit() ; e != -1 ; e = nd.first_set_bit()) {
                                ...
                                for (int w = ne.first_set_bit() ; w != -1 && unsigned(w) <= v ; w = ne.first_set_bit()) {
                                    ...
                                    if (k_ >= 3 && target_graphs.at(2 * k_ + 2).adjacent(v, w))
                                        target_graphs.at(2 * k_ + 3).add_edge(v, w);
                                    else if (k_ >= 2 && target_graphs.at(2 * k_ + 1).adjacent(v, w))
                                        target_graphs.at(2 * k_ + 2).add_edge(v, w);
                                    else if (k_ >= 1)
                                        target_graphs.at(2 * k_ + 1).add_edge(v, w);
                                }
                            }
                        }
                    }
...
auto glasgow_1_subgraph_isomorphism(const std::pair<Graph, Graph> & graphs, const Params & params) -> Result
{
    if (graphs.first.size() > graphs.second.size())
        return Result{ };
    return select_graph_size<Apply<SequentialSubgraphIsomorphism, 1, 1>::template Type, Result>(
...
auto glasgow_4_subgraph_isomorphism(const std::pair<Graph, Graph> & graphs, const Params & params) -> Result
{
    if (graphs.first.size() > graphs.second.size())
        return Result{ };
    return select_graph_size<Apply<SequentialSubgraphIsomorphism, 3, 4>::template Type, Result>(
            AllGraphSizes(), graphs.second, graphs.first, params);
}
