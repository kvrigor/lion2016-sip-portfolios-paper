...
plot \
    ...
    "lad-features.csv" u (strcol(8) eq "true" ? $7 : 1e8):(strcol(8) eq "true" ? 1 : 0) smooth cumulative with steps notitle lc rgb '#aaaaaa' dt ".", \
    "no-presolved-presolver-allinstances.csv" u (strcol(2) eq "vf2" ? $3 : NaN):($3 >= 1e8 ? 1e-10 : 1) smooth cumulative with steps notitle lc 4 dt ".", \
...
plot \
    "no-presolved-presolver-allinstances.csv" u (strcol(2) eq "lad" ? ($3<1e4?1e4:$3) : NaN):($3 >= 1e8 ? 1e-10 : 1) axes x1y2 smooth cumulative with steps notitle lc 1 dt ".", \
    "no-presolved-presolver-allinstances.csv" u (strcol(2) eq "supplementallad" ? ($3<1e4?1e4:$3) : NaN):($3 >= 1e8 ? 1e-10 : 1) axes x1y2 smooth cumulative with steps notitle dt "." lc 2, \
    "no-presolved-presolver-allinstances.csv" u (strcol(2) eq "glasgow1" ? ($3<1e4?1e4:$3) : NaN):($3 >= 1e8 ? 1e-10 : 1) axes x1y2 smooth cumulative with steps notitle dt "." lc 5, \
