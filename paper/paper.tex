...
algorithm depends on the CPU time limit considered: for very small CPU time limits, VF2
...
example, in McGregor's algorithm \cite{mcgregor79} and in VF2 \cite{Cordella:2004}.
...
\emph{AllDifferent Generalized Arc Consistency}) removes more candidate pairs than when each
...
\cite{Solnon:2010} where, for each candidate pair $(u,v)\in N_p\times N_t$, a redundant Local
AllDifferent (LAD) constraint ensures that each neighbour of $u$ may be matched with a different
...
\item By Audemard et al.\ \cite{Audemard:2014} to generalise the LAD constraint proposed by Solnon
    \cite{Solnon:2010} so that it ensures that a subset of pattern nodes can be matched with all
    different compatible target nodes, where compatibility is defined with respect to invariant
    properties;
...
\begin{itemize}
\item VF2 \cite{Cordella:2004}, which performs a weak filtering that it is especially fast on
    trivially satisfiable instances;
\item LAD \cite{Solnon:2010}, which combines two strong but expensive filtering techniques
    (AllDifferent Generalized Arc Consistency and LAD);
...
helpful, and even looking at paths of length 3 is relatively expensive on some graphs.

We also introduce two new variants of LAD. The first variant is a weaker LAD filtering (called
incompleteLAD) which is applied once, without performing a backtracking search, and very quickly
...
The second variant of LAD is called pathLAD, which combines the LAD constraints introduced by Solnon
\cite{Solnon:2010} with the exploitation of path length properties proposed by Audemard et
al.\ \cite{Audemard:2014}. The idea is to label each edge $(u,v)$ with the number of paths of length 2
...
able to solve easy unsastisfiable instances very quickly, in a few milliseconds. Hence, for time
limits lower than \SI{5}{\ms}, the best solver is incompleteLAD. However, it is not able to solve harder
unsatisfiable instances, nor can it solve satsifiable instances.

PathLAD and Glasgow(1) outperform incompleteLAD when considering longer time limits: PathLAD is the
best solver for time limits larger than \SI{5}{\ms} and lower than \SI{40}{\ms}, and Glasgow(1) is the best solver
for time limits larger than \SI{40}{\ms} and lower than \SI{3000}{\ms}.

For CPU time limits larger than \SI{3000}{\ms}, the best solver becomes Glasgow(2).  As we
increase the CPU time limit, variants of Glasgow with longer paths (Glasgow(3) and Glasgow(4))
...
monotone in the time it takes to solve the instance---Glasgow(1) is best at first, but gets worse as
the time to solve the instance increases until Glasgow(2) is better. After that point, Glasgow(1) is
never better than Glasgow(2) again. The same is true for path lengths 3 and 4.
...
running incompleteLAD, we run the VF2 solver for \SI{50}{\ms}. This solves easy instances without
the overhead of running algorithm selection and avoids potentially making incorrect solver choices.
After running incompleteLAD and VF2 for \SI{50}{\ms}, we are left with 2336 hard instances that we
...
performance across the entire set of instances, at the CPU time limit of \SI{10e8}{\ms},
i.e.\ Glasgow(2). We consider it a lower bound on performance.
...
either by incompleteLAD or VF2 presolving, as explained above. Our system is able to close more than
...
limit considered. Simple approaches like LAD preprocessing and VF2 are able to solve easy instances
very quickly, in a few milliseconds. However, they are not able to solve harder instances: LAD
preprocessing is an incomplete approach which can only detect rather trivial inconsistencies; VF2
...
and LAD preprocessing when considering longer time limits: \SI{10}{\ms} for default and new LAD and
Glasgow 1, and \SI{100}{\ms} (resp. \SI{1000}{\ms} and \SI{10000}{\ms}) for Glasgow(2) (resp.\ 3 and
...
instances than the single best (Glasgow(1)) when the limit is \SI{100}{\ms}. In many applications,
...
Add Figure 1 which compares single best at \SI{10e8}{\ms}(Glasgow(2)) with LLAMA ?
