...
exist various exact algorithms, which have been compared on a large suite of instances by
McCreesh and Prosser~\cite{McCreesh:2015}. These experiments indicated that the single best
algorithm depends on the CPU time limit considered: for very small time
...
% MODIFICATIONS
In our subgraph isomorphism context, algorithm performance is highly constrained by memory bandwidth (as pointed out by Sabharwal and Samulowitz \cite{Sabharwal2014} for SAT solvers).
Therefore, we cannot simply run different algorithms in parallel, and we consider the case where exactly one algorithm is selected for solving the problem. 
%
One of
...
\end{itemize}
% MODIFICATIONS
We have not considered the algorithm introduced in \cite{constraints10} because it is outperformed by \LAD{}.
Also, we have not considered MIP nor SAT solvers because they are not competitive with the selected algorithms \cite{UpcomingIJCAIPaper}.
% 
...
features as features. However, those features were not more informative than the
other ones.
...
% MODIFICATIONS: removed decimals (just keep to digits after the point)
\begin{enumerate}
    \item If \IncompleteLAD presolving removes at least
        \SI{28.01}{\percent} of the pairs, and at least
        \SI{94.12}{\percent} of the values from at least one domain, then
        pick \PathLAD.
    ...
    \item If \IncompleteLAD filtering removed at least \SI{5.90}{\percent} of
        possible pairs, and if less than \SI{84.66}{\percent} of the pattern
